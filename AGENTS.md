...
5. **Bootstrap-first styling policy**: Bootstrap CSS must be the default starting point for component layout, structure, spacing, typography, forms, buttons, grids, cards, badges, alerts, and responsive behavior. Prefer Bootstrap classes in the template first, then use Bootstrap SCSS variables, mixins, and utilities for customizations. Build the UI using Bootstrap before adding any custom CSS.
6. **SCSS and Bootstrap integration**: Use the SCSS version of Bootstrap as the design foundation. Override Bootstrap tokens and theme variables in `src/styles.scss` before importing Bootstrap, and use Bootstrap SCSS mixins/functions/variables wherever custom styling is needed. Write Sass-based styling whenever custom work is required, but keep it minimal and scoped.
7. **When custom CSS is necessary**: Only write component-level CSS when Bootstrap does not provide the needed behavior or visual treatment. Keep custom CSS small, specific, and SCSS-based; prefer Bootstrap utility classes or Bootstrap component modifiers first. If a UI change is layered on top of Bootstrap, implement it as a Bootstrap-aware override using utility classes, theme variables, or minimal SCSS, not ad hoc raw CSS.
8. **Component design workflow**: For every new component or UI change, first structure the markup using Bootstrap grid, spacing utilities, form controls, buttons, and layout classes. If a custom look is needed, extend Bootstrap with SCSS variables or small SCSS rules in the component file only after the Bootstrap structure is in place. Do not start by writing raw CSS for standard layout patterns.
